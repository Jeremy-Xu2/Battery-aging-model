...
OCVcell = csvread(namecell);        % the user-supplied measured OCV curve of the cell (supplied by the user)
OCVn = csvread(nameneg);            % the user-supplied anode OCV curve
OCVp = csvread(namepos);            % the user-supplied cathode OCV curve
OCVsim = csvread(nameOCV);          % the simulated OCV curve (simulated by the code)
