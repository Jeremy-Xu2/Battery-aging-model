...
Cell_user::Cell_user(const Model& MM, int verbosei){
	/*
	...
	Vmin = 2.5;							// value for an NMC/C cell
	...
	// OCV curves
	string namepos = "LGChem_OCV_NMC.csv"; 						// name of the csv file with the cathode OCV curve
	string nameneg = "LGChem_OCV_C.csv";							// name of the csv file with the anode OCV curve
	...
	if(OCV_pos_n >= OCV_maxLength){
		cerr<<"ERROR in the constructor of Cell_user. The cathode OCV curve has "<<OCV_pos_n<<" points while the maximum number is "<<OCV_maxLength<<". Throwing an error"<<endl<<flush;
		leng = true;
	}
	if(OCV_neg_n >= OCV_maxLength){
		cerr<<"ERROR in the constructor of Cell_user. The anode OCV curve has "<<OCV_neg_n<<" points while the maximum number is "<<OCV_maxLength<<". Throwing an error"<<endl<<flush;
		leng = true;
	}
	if(dOCV_neg_n >= OCV_maxLength){
		cerr<<"ERROR in the constructor of Cell_user. The anode entropic coefficient curve has "<<dOCV_neg_n<<" points while the maximum number is "<<OCV_maxLength<<". Throwing an error"<<endl<<flush;
		leng = true;
	}
	if(dOCV_tot_n >= OCV_maxLength){
		cerr<<"ERROR in the constructor of Cell_user. The cell entropic coefficient curve has "<<dOCV_tot_n<<" points while the maximum number is "<<OCV_maxLength<<". Throwing an error"<<endl<<flush;
		leng = true;
	}
	if(leng)
		throw 111;
	...
	try{
		...
		cout<<"ERROR in Cell_KokamNMC::Cell_user when loading the OCV curves from the CSV files: "<<e<<". Throwing it on"<<endl<<flush;
		throw e;
	}
	...
	if (Mnch)
		cerr<<"ERROR in Cell_user::Cell_user: the value of nch used in the Matlab script "<<M.Input[0] <<" is not the same as the value of nch used in the c++ code "<<nch <<endl<<flush;
	bool Mrp = (M.Input[1]-Rp)/M.Input[1] > pow(10,-10);		// allow a relative difference of e-10 due to numerical errors
	if (Mrp)
		cerr<<"ERROR in Cell_user::Cell_user: the value of Rp used in the Matlab script "<<M.Input[1] <<" is not the same as the value of Rp used in the c++ code "<<Rp <<endl<<flush;
	bool Mrn = (M.Input[2]-Rn)/M.Input[2] > pow(10,-10);		// allow a relative difference of e-10 due to numerical errors
	if (Mrn)
		cerr<<"ERROR in Cell_user::Cell_user: the value of Rn used in the Matlab script "<<M.Input[2] <<" is not the same as the value of Rn used in the c++ code "<<Rn <<endl<<flush;
	int a = M.Input[3];
	bool Meig = abs(M.An[a])> pow(10,-10) || abs(M.Ap[a])> pow(10,-10); // allow a relative difference of e-10 due to numerical errors
	if (Meig)
		cerr<<"ERROR in Cell_user::Cell_user: the row of the 0-eigenvalue is "<<M.Input[3] <<" but that row has a positive eigenvalue of "<<M.Ap[a]<<" and negative eigenvalue of "<<M.An[a]<<". They are not 0." <<endl;
	...
	fp 		= 0.58725;				// lithium fraction in the cathode at 50% soc [-], OCV fitting parameter
	fn 		= 0.46465;				// lithium fraction in the anode at 50% soc [-], OCV fitting parameter
	...
	thickn 	= 1.170972150305478*pow(10,-4);		// thickness of the negative electrode, OCV fitting parameter
	...
	Dp 		= 2.784387*pow(10,-13);		// diffusion constant of the cathode at reference temperature, characterisation fitting parameter
	Dn 		= 5.071985*pow(10,-13);		// diffusion constant of the anode at reference temperature, characterisation fitting parameter
	...
	// LAM
		OCVnmc = 4.1;
		// fitting parameters
		lamparam.lam1p = 3.4985*pow(10,-9);
		lamparam.lam1n = 5.88*pow(10,-13);
...
Cell_user::Cell_user(const Model& M, const DEG_ID& degid, int verbosei) : Cell_user(M, verbosei) {
	/*
