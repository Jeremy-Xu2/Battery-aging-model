...
int main(int argv, char* argc[]){
	...
	Model M;
	Model_initialise(M);
	cout<<"after Model_initialise"<<endl;
	...
	int cellType = 1; 				// which cell to use for the simulation.
...
//	duration = ( std::clock() - tstart ) / (double) CLOCKS_PER_SEC;
	cout<<"finished all simulations in "<<endl;
