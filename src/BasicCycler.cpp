...
//		else if (cps <= 5000 || cps > 50000){					// the cell has exceeded the minimum voltage limit
//			c.setStates(s2, Iprev);
//			endcriterion = -4;
//			break;
//		}
//		else if (cns <= 500 || cns > 29000){					// the cell has exceeded the minimum voltage limit
//			c.setStates(s2, Iprev);
//			endcriterion = -4;
//			break;
//		}
...
void BasicCycler::CC_V_CV_I(double Crate, double Vset, double Ccut, double dt, bool blockDegradation, double* ahi, double* whi, double* timei){
	...
	try{
		...
		endcr = CC_V(sign*Crate*c.getNominalCap(), dt, blockDegradation, Vset, &ah1, &wh1, &tt1); // CC (dis)charge at the given Crate
	...
	if(Ccut < Crate && endcr != -4){											// cutoff current is larger than the CC current, so we don't have to do a CV phase
