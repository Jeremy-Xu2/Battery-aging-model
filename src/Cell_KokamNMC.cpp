...
Cell_KokamNMC::Cell_KokamNMC(const Model& MM, int verbosei){
	/*
	...
	kp_T = 58000;
	kn = 1.7640*pow(10,-11);			// fitting parameter
	kn_T = 20000;
	...
	if(OCV_pos_n >= OCV_maxLength){
		cerr<<"ERROR in the constructor of Cell_KokamNMC. The cathode OCV curve has "<<OCV_pos_n<<" points while the maximum number is "<<OCV_maxLength<<". Throwing an error"<<endl<<flush;
		leng = true;
	}
	if(OCV_neg_n >= OCV_maxLength){
		cerr<<"ERROR in the constructor of Cell_KokamNMC. The anode OCV curve has "<<OCV_neg_n<<" points while the maximum number is "<<OCV_maxLength<<". Throwing an error"<<endl<<flush;
		leng = true;
	}
	if(dOCV_neg_n >= OCV_maxLength){
		cerr<<"ERROR in the constructor of Cell_KokamNMC. The anode entropic coefficient curve has "<<dOCV_neg_n<<" points while the maximum number is "<<OCV_maxLength<<". Throwing an error"<<endl<<flush;
		leng = true;
	}
	if(dOCV_tot_n >= OCV_maxLength){
		cerr<<"ERROR in the constructor of Cell_KokamNMC. The cell entropic coefficient curve has "<<dOCV_tot_n<<" points while the maximum number is "<<OCV_maxLength<<". Throwing an error"<<endl<<flush;
		leng = true;
	}
	if(leng)
		throw 111;
	...
	try{
		...
		cout<<"ERROR in Cell_KokamNMC::Cell_KokamNMC when loading the OCV curves from the CSV files: "<<e<<". Throwing it on"<<endl<<flush;
		throw e;
	}
	...
	if (Mnch)
		cerr<<"ERROR in Cell_KokamNMC::Cell_KokamNMC: the value of nch used in the Matlab script "<<M.Input[0] <<" is not the same as the value of nch used in the c++ code "<<nch <<endl<<flush;
	bool Mrp = (M.Input[1]-Rp)/M.Input[1] > pow(10,-10);		// allow a relative difference of e-10 due to numerical errors
	if (Mrp)
		cerr<<"ERROR in Cell_KokamNMC::Cell_KokamNMC: the value of Rp used in the Matlab script "<<M.Input[1] <<" is not the same as the value of Rp used in the c++ code "<<Rp <<endl<<flush;
	bool Mrn = (M.Input[2]-Rn)/M.Input[2] > pow(10,-10);		// allow a relative difference of e-10 due to numerical errors
	if (Mrn)
		cerr<<"ERROR in Cell_KokamNMC::Cell_KokamNMC: the value of Rn used in the Matlab script "<<M.Input[2] <<" is not the same as the value of Rn used in the c++ code "<<Rn <<endl<<flush;
	int a = M.Input[3];
	bool Meig = abs(M.An[a])> pow(10,-10) || abs(M.Ap[a])> pow(10,-10); // allow a relative difference of e-10 due to numerical errors
	if (Meig)
		cerr<<"ERROR in Cell_KokamNMC::Cell_KokamNMC: the row of the 0-eigenvalue is "<<M.Input[3] <<" but that row has a positive eigenvalue of "<<M.Ap[a]<<" and negative eigenvalue of "<<M.An[a]<<". They are not 0." <<endl;
...
Cell_KokamNMC::Cell_KokamNMC(const Model& M, const DEG_ID& degid, int verbosei) : Cell_KokamNMC(M, verbosei) {
	/*
