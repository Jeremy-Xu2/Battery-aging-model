...
void estimateCharacterisation(){
	...
	int nCCCV = 3;														// number of data sets for CCCV (dis)charges
	string names[nCCCV] = {"Characterisation_0.5C_CC_discharge.csv",
			"Characterisation_1C_CC_discharge.csv", "Characterisation_2C_CC_discharge.csv"};// Name of the files with the voltage curve from the cell
	int lengths[nCCCV] = {864,344,174};							// the number of data points for each voltage curve
	double Crates[nCCCV] = {0.5, 1, 2};							// C rates of the CC phases for each voltage curve, <0 for charge, >0 for discharge
	double Ccuts[nCCCV] = {100, 100, 100};					// C rates of the current threshold for the CV phases [A], > 0
																		// 	a very high value (above the Crate of the CC) will avoid there is a CV phase at all, so there is only a CC phase
	double weights[nCCCV] = {0.2, 0.2, 0.2};					// array with the weight that should be attributed to each individual CCCV curve when calculating the overall error
	...
	OCVparam ocvfit;
		...
		ocvfit.thickp = 75.2*pow(10,-6);									// thickness of the cathode
		ocvfit.thickn = 85.2*pow(10,-6);								// thickness of the anode
		ocvfit.lifracpini = 0.58725;										// lithium fraction in the cathode at 50% soC
		ocvfit.lifracnini = 0.46465;										// lithium fraction in the anode at 50% SoC
		...
		ocvfit.cap = 4.87;												// the capacity of the cell [Ah]
		ocvfit.Vmax = 4.2;												// maximum voltage of the cell [V]
		ocvfit.Vmin = 2.5; 												// minimum voltage of the cell [V]
		...
		ocvfit.np = 317;													// number of points in the cathode OCV curve
		ocvfit.nn = 428;													// number of points in the anode OCV curve
