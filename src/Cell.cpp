...
bool Cell::getVoltage(bool print, double* V, double* OCVp, double* OCVn, double* etap, double* etan, double* Rdrop, double* Temp){
	...
	if (cps <= 0 || cns <= 0 || cps >= Cmaxpos || cns >= Cmaxneg){
		if (print){		// print error message unless you want to suppress the output
			...
			cerr<<"ERROR in Cell::getVoltage: concentration out of bounds. the positive lithium fraction is "<<cps<<" and the negative lithium fraction is "<<cns;
...
//		cout<<"the positive lithium fraction is "<<cps<<" and the negative lithium fraction is "<<cns<<endl<<flush;
